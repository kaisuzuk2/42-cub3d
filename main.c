...
/*   Updated: 2026/02/02 10:25:55 by kaisuzuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	parse_arg(int argc, char **argv)
{
	...
	file_name = argv[1];
	if (ft_strncmp(file_name + (len - CUB_EXT_LEN), CUB_FILE_EXT,
			ft_strlen(CUB_FILE_EXT)))
	{
...
int	main(int argc, char *argv[])
{
	...
	init_config(&game.conf);
	init_game(&game);
	if (!read_file(argv[1], &game.conf) || !init(&game))
	{
		dispose_conf(&game.conf);
		dispose_game(&game);
		return (EXIT_FAILUE);
