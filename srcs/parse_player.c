...
/*   Updated: 2026/01/27 15:30:44 by kaisuzuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	register_player_at(t_config *conf, int x, int y)
{
	...
	{
		print_error(MAP_LABEL, "multiple player positions.");
		return (FALSE);
	}
...
t_bool	setup_player_from_map(t_config *conf)
{
	...
	while (conf->map && conf->map[y])
	{
		...
		while (conf->map[y][x])
		{
			if (!is_valid_map_char(conf->map[y][x]))
				return (print_error_detail(MAP_LABEL, "invalid map character",
						&conf->map[y][x]), FALSE);
	...
	if (conf->player_dir_char == 0)
		return (print_error(MAP_LABEL, "player not found."), FALSE);
